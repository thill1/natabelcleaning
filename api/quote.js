...
function emailHtml(body, quote, customer) {
  const move = String(body.service_type || '') === 'move';
  const amount = `$${Number(quote.amount).toLocaleString()}`;
  const cadence = move ? 'one-time cleaning' : 'per visit';
  const firstVisitNote = move
    ? 'This Move-In / Move-Out price includes the approved $175 move-service premium for the home’s square-footage tier.'
    : String(body.recent_cleaning || '') === 'no'
  ...
  const serviceLine = move
    ? `Move-In / Move-Out cleaning for ${safe(body.name || 'this home')} in ${safe(body.city)} ${safe(body.zip)}.`
    : `<strong>${safe(frequencyLabel(body.frequency))}</strong> recurring cleaning for ${safe(body.name || 'this home')} in ${safe(body.city)} ${safe(body.zip)}.`;
  const recentLine = move ? '' : `<br><strong>Professionally cleaned in the last 30 days:</strong> ${safe(body.recent_cleaning)}`;
  ...
  return `<div style="font-family:Arial,sans-serif;color:#17140f;line-height:1.6;max-width:620px;margin:auto">
    ...
    <h1 style="font-family:Georgia,serif;font-weight:500">${customer ? (move ? 'Your Move-In / Move-Out cleaning quote' : 'Your recurring cleaning quote') : 'New instant-quote lead'}</h1>
    <p style="font-size:38px;font-family:Georgia,serif;margin:18px 0">${amount} <span style="font-size:16px">${cadence}</span></p>
    <p><strong>Cleaning service only.</strong> Prices are subject to change.</p>
    <p>${serviceLine}</p>
    <p><strong>Home:</strong> ${safe(body.square_footage)} sq ft · ${safe(body.bedrooms)} bed · ${safe(body.bathrooms)} bath<br><strong>Property:</strong> ${safe(body.property_type)}<br><strong>Service address:</strong> ${safe(body.service_address)}${recentLine}</p>
    ...
    ${customer ? '<p>NataBel will follow up using the contact details you provided to confirm scope and scheduling. If you would like immediate help, call (916) 899-8811.</p>' : `<p><strong>Phone:</strong> ${safe(body.phone)}<br><strong>Email:</strong> ${safe(body.email)}<br><strong>Notes:</strong> ${safe(body.notes)}</p>`}
  </div>`;
}
...
module.exports = async function handler(req, res) {
  ...
  if (body.preview === true) return res.status(200).json({ ok: true, status: 'estimated', quote: result.quote });
  if (!validContact(body)) return res.status(400).json({ ok: false, error: 'invalid_contact' });

  const move = String(body.service_type || '') === 'move';
  const customer = await sendEmail({
    to: body.email,
    subject: move ? `Your NataBel Move-In / Move-Out quote: $${result.quote.amount}` : `Your NataBel recurring quote: $${result.quote.amount} per visit`,
    html: emailHtml(body, result.quote, true)
  });
  const internal = await sendEmail({
    to: process.env.LEAD_TO_EMAIL || BUSINESS_EMAIL,
    subject: `New NataBel ${move ? 'Move-In / Move-Out' : 'recurring'} quote — ${body.name}`,
